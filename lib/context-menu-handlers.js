...
const contextMenuVideoContentScript = `
self.on('click', function (node, data) {
  self.postMessage({
    src: node.href || node.src,
    url: window.location.href,
    domain: new URL(node.href || node.src).host
  });
});`;
...
function init() {
  ...
  const onMessageVideoHandler = (opts) => {
  ...
  getItem({
    ...
    contentScript: contextMenuVideoContentScript,
    onMessage: onMessageVideoHandler
  });
